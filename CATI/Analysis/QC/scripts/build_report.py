...
def build_summary(wb):
    ...
    # Headers — dynamic so adding rounds (R6, R7, ...) doesn't break the layout
    ws.row_dimensions[3].height = 32
    new_in_labels = [f'New in {r}' for r in ROUNDS[1:]]
    headers = (['Module', 'Module Name']
               + [ROUND_LABELS[r] for r in ROUNDS]
               + new_in_labels
               + ['Dropped','Changed','Notes'])
    ...
    # Round-count columns
    n_rounds = len(ROUNDS)
    for ci in range(3, 3 + n_rounds):
        ws.column_dimensions[get_column_letter(ci)].width = 14
    # "New in Rn" + Dropped + Changed + Notes
    n_trailing = len(new_in_labels) + 3
    for ci in range(3 + n_rounds, 3 + n_rounds + n_trailing):
        ws.column_dimensions[get_column_letter(ci)].width = 12
    # Notes col gets wide
    ws.column_dimensions[get_column_letter(2 + n_rounds + n_trailing)].width = 40
    ...
    for i, mod in enumerate(MODULES):
        ...
        row_data = (
            [mod, MOD_NAMES[mod]]
            + [counts_by_round[r] for r in ROUNDS]
            + [new_counts.get(r, 0) for r in ROUNDS[1:]]
            + [dropped, changed, '']
        )
