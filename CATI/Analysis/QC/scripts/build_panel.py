...
def _load_passport_with_design():
    """
    Load l2phl_M00_passport.dta and merge survey-design vars (psu, stratum,
    urban) from l2phl_cati_household.dta if M00 lacks them.

    The M00 passport pool deliberately does NOT merge psu/stratum/urban/popw/hhw
    (see Section 3G in hf_l2phl_M00_passport@Claude — merge block commented out).
    Pull them from l2phl_cati_household, which has all of them for R1–R7.

    Also derives `region` from the first 2 chars of psu.
    """
    pp = pd.read_stata(
        _os.path.join(_HF, 'l2phl_M00_passport.dta'),
        convert_categoricals=False
    )
    have_design = set(['psu','stratum','urban']).issubset(pp.columns)
    if not have_design:
        hh_path = _os.path.join(_HF, 'l2phl_cati_household.dta')
        if _os.path.exists(hh_path):
            hhd = pd.read_stata(hh_path, convert_categoricals=False)
            hhd = hhd[['round','hhid','psu','stratum','urban']].drop_duplicates(
                subset=['round','hhid'])
            pp = pp.merge(hhd, on=['round','hhid'], how='left',
                          suffixes=('','_hh'))
    # Derive region code from first 2 chars of psu (string-safe)
    pp['region'] = pp['psu'].astype(str).str[:2]
    pp['region'] = pd.to_numeric(pp['region'], errors='coerce').astype('Int64')
    return pp


def load_data():
    pp = _load_passport_with_design()
    ...
    # After the 2026-04-03 M00 pooling do-file revision, all rounds save to
    # `date_of_interview` (R01's `start_date` is renamed upstream). Keep a
    # cap-and-fallback so legacy pools with `start_date` still load.
    if 'start_date' in pp.columns:
        pp['int_date'] = pp['start_date'].combine_first(pp['date_of_interview'])
    else:
        pp['int_date'] = pp['date_of_interview']
