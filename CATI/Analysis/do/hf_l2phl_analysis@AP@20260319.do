...
// 		bys stratum (psu hhid): egen stratum_nhh = total(hhw)
	...
	//Open baseline roster
		use "$hf/R00/l2phl_20251015_M01_roster.dta", clear
			merge m:1 hhid using "$hf/R00/l2phl_20251015_M00_passport.dta", ///
				assert(3) nogen keepusing(stg_id)

		keep hhid fmid hhsize age gender stg_id 
		...
		bys hhid (fmid): g N = _N 
			...
			merge 1:1 hhid fmid using `indw', assert(3) nogen ///
				keepusing(indw region urban stratum* pop_cell age_grp )  

			tab region, g(region_)
			tab stratum, g(stratum_)
// 			keep hhid stg_id locale hhsize region region_* stratum stratum_* 			
		
			tempfile base_roster 
			save `base_roster'	, replace 
