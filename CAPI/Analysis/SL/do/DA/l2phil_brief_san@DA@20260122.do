...
use "$in/l2phl_${date}_M11_san.dta", clear
...
label var macro_region "Macro-region" 
 ...
 forvalues k = 1/5 {
    gen choice_`k' = (strpos(s5_txt, ";`k';") > 0)
}

recode choice* (0=.) if s5_txt==""

label variable choice_1 "Waste segregation category: Organic (food/yard)"
label variable choice_2 "Waste segregation category: Recyclables (plastic, paper, metal, glass"
label variable choice_3 "Waste segregation category: Hazardous (batteries, chemicals)"
label variable choice_4 "Waste segregation category: Medical waste"
label variable choice_5 "Waste segregation category: None"
...
replace s4=96 if s4>11&s4<20

* merge 1:1 hhid using "$in/${dta_file}_M09_nh.dta", assert(3) keep(3) nogen 


table s1 , stat(percent)
collect export "$out/Phil_Brief_Sanit_Graphs_${date}", ///
			as(xlsx) sheet("s1") cell(A1) replace	

local varlist s1 s2 s3 s4 choice_1 choice_2 choice_3 choice_4 choice_5 s6 s7 s8
...
foreach var of local varlist {
  ...
  collect export "$out/Phil_Brief_Sanit_Graphs_${date}", as(xlsx) sheet(urban_sanit) modify
...
foreach var of local varlist {
  ...
  collect layout (`varlist') (locale)
  collect export "$out/Phil_Brief_Sanit_Graphs_${date}", as(xlsx) sheet(urban_sanit_num) modify
...
foreach var of local varlist {
  ...
  collect export "$out/Phil_Brief_Sanit_Graphs_${date}", as(xlsx) sheet(macro_region_sanit) modify
...
foreach var of local varlist {
  ...
  collect layout (`varlist') (macro_region)
  collect export "$out/Phil_Brief_Sanit_Graphs_${date}", as(xlsx) sheet(macro_region_sanit_num) modify

table (locale) [aw= hhw] ,  stat(mean s7a)   nformat(%9.2f mean) name(c1) replace
table (locale), ///
    statistic(count s7a) ///
    statistic(min s7a) ///
    statistic(max s7a) ///	
...
collect layout (colname # result) (locale)
		...
		collect export "$out/Phil_Brief_Sanit_Graphs_${date}", ///
			name(c1) as(xlsx) sheet("waste_cost") cell(A1) modify	
