...
table v1 , stat(percent)
collect export "$out/Phil_Brief_View_Graphs_${date}", ///
			as(xlsx) sheet("v1") cell(A1) replace	

local varlist v1 v2 v3 v4 v5 v6 v7 v8 v9a v9b v9c v9d v9e v9f v9g v9h v9i v9j v9k v10a v10b v10c v10d v10e
...
foreach var of local varlist {
  ...
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(urban_view) modify
...
foreach var of local varlist {
  ...
  collect layout (`varlist') (locale)
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(urban_view_num) modify
...
foreach var of local varlist {
  ...
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(macro_region_view) modify
...
foreach var of local varlist {
  ...
  collect layout (`varlist') (macro_region)
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(macro_region_view_num) modify
  
 
/*
 collect clear
foreach var of local varlist {
      table (locale),  name(Table) append  stat(fvpercent `var') 
     }

  collect layout (`varlist') (locale)
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(urban_view_noweight) modify

  
foreach var of local varlist {
     table (macro_region),  name(Table) append  stat(fvpercent `var') 
     }

  collect layout (`varlist') (macro_region)
  collect export "$out/Phil_Brief_View_Graphs_${date}", as(xlsx) sheet(macro_region_view_noweight) modify
